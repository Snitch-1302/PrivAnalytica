...
async def predict_logistic_regression(request: EncryptedFeaturesRequest):
    ...
    try:
        ...
        is_medical_data = (
            "medical" in dataset_name or 
            "medical" in dataset_type or
            dataset_name.startswith("medical") or
            dataset_type == "medical"
        )
        
        if not is_medical_data and request.metadata and not request.metadata.get("demo", False):
            raise HTTPException(
                status_code=400, 
                detail="Logistic regression model only works with medical data. "
...
async def predict_linear_regression(request: EncryptedFeaturesRequest):
    ...
    try:
        ...
        if not is_medical_data and request.metadata and not request.metadata.get("demo", False):
            raise HTTPException(
                status_code=400, 
                detail="Linear regression model only works with medical data. "
                       "This model requires features: age, blood_pressure, cholesterol. "
                       f"Received dataset: {dataset_name or 'unknown'}"
            )
